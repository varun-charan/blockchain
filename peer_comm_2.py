...
port_list2 = [4444, 5555]
port = 6666

class Receiver2(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self, name="Block receiver2")
...
class Sender2(threading.Thread):

    def __init__(self):
        threading.Thread.__init__(self, name="Block sender2")

    def send(self, message):
        print("sending blockchain to peers")
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            for item in port_list2:
...
def initialize2():
    receiver2 = Receiver2()
    sender2 = Sender2()
    threads1 = [receiver2.start(), sender2.start()]
